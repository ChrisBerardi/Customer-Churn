...
Of more important is the identification of features that give rise to churn, specifically: Contract, TotalCharges, MonthlyCharges. 
...
The final important feature, MonthlyCharges, is the last box-plot on the preceding page. Customers that churn spend, on average, more than those who don't. While we expect customers that pay to want to find different service, the reason why they pay more must be found. The interpretation of this plot is more complicated when the previous two plots are considered. As has been already established, customers that churn generally do not sign contracts, increasing the average amount they pay. This accounts for much of the increased charges. FINISH

Taken together, the model, as well as the feature importance, show a path that can be taken to minimize future churn. FINISH
